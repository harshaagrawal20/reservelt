            ...
            <div className="flex gap-3">
              {unreadCount > 0 && (
                <button
                  ...
                  className="px-6 py-3 bg-blue-600 text-white text-sm font-medium rounded-xl hover:bg-blue-700 hover:shadow-lg hover:scale-105 transition-all duration-200 transform"
                >
                  Mark All Read
                </button>
              )}
              <button
                ...
                className="px-6 py-3 border border-gray-300 text-gray-700 text-sm font-medium rounded-xl hover:bg-gray-50 hover:border-gray-400 hover:shadow-lg hover:scale-105 transition-all duration-200 transform"
                disabled={isLoading}
              >
                {isLoading ? "🔄" : "🔄"} Refresh
              </button>
              ...
              <div className="flex items-center justify-between">
                ...
                <div className="flex gap-3">
                  <button
                    ...
                    className="px-5 py-2.5 bg-blue-600 text-white text-sm font-medium rounded-xl hover:bg-blue-700 hover:shadow-lg hover:scale-105 transition-all duration-200 transform"
                  >
                    Mark Read
                  </button>
                  <button
                    onClick={() => handleBulkAction("delete")}
                    className="px-5 py-2.5 bg-red-600 text-white text-sm font-medium rounded-xl hover:bg-red-700 hover:shadow-lg hover:scale-105 transition-all duration-200 transform"
                  >
                    Delete
                  </button>
                  <button
                    onClick={() => setSelectedNotifications(new Set())}
                    className="px-5 py-2.5 bg-gray-600 text-white text-sm font-medium rounded-xl hover:bg-gray-700 hover:shadow-lg hover:scale-105 transition-all duration-200 transform"
                  >
                    Clear
                  </button>
                </div>
                ...
                onChange={(e) => {
                  if (e.target.checked) {
                    setSelectedNotifications(
                      new Set(filteredAndSortedNotifications.map((n) => n._id))
                    );
                  } else {
                    setSelectedNotifications(new Set());
                  }
        ...
        return (
          <>
            <button
              ...
              className="text-sm text-white font-medium bg-green-600 hover:bg-green-700 hover:shadow-lg hover:scale-105 px-4 py-2.5 rounded-xl transition-all duration-200 transform"
              disabled={isLoading}
            >
              Accept
            </button>
            <button
              ...
              className="text-sm text-white font-medium bg-red-600 hover:bg-red-700 hover:shadow-lg hover:scale-105 px-4 py-2.5 rounded-xl transition-all duration-200 transform"
              disabled={isLoading}
            >
              Decline
            </button>
...
const NotificationCard = ({
  ...
  return (
    <div
      className={`bg-white rounded-lg shadow-sm border-l-4 border-${style.color}-500 hover:shadow-md transition-shadow ${style.opacity}`}
    >
      <div className="p-4">
        <div className="flex items-start justify-between">
          <div className="flex items-start space-x-3 flex-1">
            ...
            {/* Notification Icon */}
            <div
              className={`w-10 h-10 rounded-full ${style.bgColor} flex items-center justify-center text-lg`}
            >
              {style.icon}
            </div>
            ...
            <div className="flex-1 min-w-0">
              ...
              <div className="mt-3 flex items-center space-x-3">
                <button
                  ...
                  className="text-sm text-white font-medium bg-black hover:bg-gray hover:shadow-lg hover:scale-105 px-4 py-2.5 rounded-xl transition-all duration-200 transform"
                >
                  View Details
                </button>
      ...
      <div className="bg-white rounded-lg shadow-xl w-full max-w-2xl max-h-[90vh] overflow-hidden">
        <div className="p-6 border-b flex items-center justify-between">
          <div className="flex items-center space-x-3">
            <div
              className={`w-12 h-12 rounded-full ${style.bgColor} flex items-center justify-center text-xl`}
            >
              {style.icon}
            </div>
            <div>
        ...
        <div className="p-6 border-t bg-gray-50 flex justify-end space-x-4">
          <button
            onClick={onClose}
            className="px-6 py-3 border border-gray-300 text-gray-700 font-medium rounded-xl hover:bg-gray-50 hover:border-gray-400 hover:shadow-lg hover:scale-105 transition-all duration-200 transform"
          >
            Close
          </button>
          {!notification.isRead && (
            <button
              ...
              className="px-6 py-3 bg-green-600 text-white font-medium rounded-xl hover:bg-green-700 hover:shadow-lg hover:scale-105 transition-all duration-200 transform"
            >
              Mark as Read
            </button>
          )}
          <button
            onClick={onDelete}
            className="px-6 py-3 bg-red-600 text-white font-medium rounded-xl hover:bg-red-700 hover:shadow-lg hover:scale-105 transition-all duration-200 transform"
          >
            Delete
          </button>
        </div>
  ...
  switch (notification.type) {
      ...
      return (
        <span>
          ...
          {metadata.scheduledDate
            ? new Date(metadata.scheduledDate).toLocaleDateString()
            : "Date TBD"}
        </span>
      );
    case "payment":
    ...
    case "due_payment":
      return (
        <span>
          💰 ${metadata.amount} •{" "}
          {metadata.method || "Payment method not specified"}
        </span>
      );
    case "rental_request":
      return (
        <span>
          ...
          {metadata.duration || "Duration not specified"}
        </span>
      );
    default:
      return null;
